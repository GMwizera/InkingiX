# InkingiX Rwanda — Master Build Prompt
...
> It contains everything needed to build, run, and deploy InkingiX Rwanda from
...
You are a senior full-stack web developer building **InkingiX Rwanda** — a
...
InkingiX Rwanda replaces the missing counselor with a free, accessible digital
tool.
...
CREATE DATABASE IF NOT EXISTS InkingiX CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE InkingiX;
...
DB_PASS=
APP_ENV=development
APP_URL=http://localhost/InkingiX-rwanda/public
```
...
- Logo: text-based "InkingiX" with a small bridge/arrow icon
...
git remote add origin https://github.com/USERNAME/InkingiX-rwanda.git
git branch -M main
...
# Clone repo
cd /var/www/html
git clone https://github.com/USERNAME/InkingiX-rwanda.git
cd InkingiX-rwanda

# Create .env (never copy from repo)
nano .env

# Set Apache document root to /var/www/html/InkingiX-rwanda/public
sudo nano /etc/apache2/sites-available/InkingiX.conf
sudo a2ensite InkingiX.conf
    ...
    steps:
      - name: SSH deploy
        ...
        with:
          ...
          script: |
            cd /var/www/html/InkingiX-rwanda
...
2. Upload to `/public_html/InkingiX/` via cPanel File Manager
...
6. Set subdomain document root to `/public_html/InkingiX/public/`
...
_Document version: 1.0 — InkingiX Rwanda_
_Prepared for: AI-assisted development handoff_
_Author: Gisele Mwizera Amen / ESICIA Ltd_
_Stack: PHP 8 · MySQL 8 · Apache · Bootstrap 5 · Vanilla JS_
